...
本文件是模块配置模板。只填门禁键，不写行为事实。无模块时 discovery 不创建；harvest 时才复制为 `profiles/<module_id>.md`。预算见 `global_rules/kv_budget.md` §3。
...
- 1.1.2（2026-09-16）：只填门禁键；事实写 standards。
- 1.1.1（2026-09-12）：discovery 不建配置；harvest 时才复制。
