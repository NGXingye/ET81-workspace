...
- [`md_governance.md`](./md_governance.md)  
  ...
  读取时机：创建或改写受管理 Markdown 前。
- [`kv_budget.md`](./kv_budget.md)  
  职责：游标、模块配置、开工契约的键值最小集、上限与违规处置。不是章节标题表。  
  读取时机：改这三种文件的键值项前。
- [`code_governance.md`](./code_governance.md)  
  职责：工具与 trunk C# 读写门禁、SVN 更新与授权提交。不含 C# 细规。  
  读取时机：改代码或 tools 前。
- [`tools_code.md`](./tools_code.md)  
  职责：`tools/` 脚本架构、硬编码、体积与拆分处置。  
  读取时机：新增或改 tools 脚本前。

# 2. 版本历史

- 1.4.0（2026-09-07）：登记 `tools_code.md`。
